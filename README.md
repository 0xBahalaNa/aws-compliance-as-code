...
| Control / Component | CJIS v6.1 Delta | FedRAMP Baseline | NIST 800-53 Rev. 5 |
|---|---|---|---|
| Deny Audit Log Deletion | n/a: no delta beyond the 800-53 requirement | AU-9 (L/M/H), AU-12 (L/M/H) | AU-9 (Protection of Audit Information), AU-12 (Audit Record Generation) |
| Deny Root User Usage | n/a: no delta beyond the 800-53 requirement | AC-6 (M/H), AC-3 (L/M/H) | AC-6 (Least Privilege), AC-3 (Access Enforcement) |
| Protect Logging and Encryption | n/a: no delta beyond the 800-53 requirement | AU-9 (L/M/H), SC-28 (M/H) | AU-9 (Protection of Audit Information), SC-28 (Protection of Information at Rest) |
| Require S3 KMS Encryption | Enforces SSE-KMS, the precondition for the SC-28(1) agency-managed-key requirement for CJI at rest; the AWS-managed `aws/s3` key alone does not satisfy it | SC-28 (M/H), SC-13 (L/M/H) | SC-28 (Protection of Information at Rest), SC-13 (Cryptographic Protection) |
| Restrict Network Changes by Region | CJI boundary protection: confines security-group changes to approved regions so CJI traffic cannot be redirected into unapproved boundaries (see CJIS v6.1 Relevance below) | SC-7 (L/M/H) | SC-7 (Boundary Protection) |
| Layer 1: Logging and Monitoring | Partial: Object Lock integrity controls in place; S3 log store closes CJI-at-rest by passing the Layer 3 agency-managed CMK, but the CloudWatch hot-mirror stays on AWS-managed encryption; full SC-28(1) closure deferred to v1.2.0 | AU-2 (L/M/H), AU-3 (L/M/H), AU-9 (L/M/H), AU-12 (L/M/H) | AU-2 (Event Logging), AU-3 (Content of Audit Records), AU-9 (Protection of Audit Information), AU-12 (Audit Record Generation) |
| Layer 2: IAM Baseline | Partial IA-5: password policy in place, but CJIS AAL2 MFA (IA-2) is not met here; deferred to Identity Center | AC-2 (L/M/H), AC-3 (L/M/H), AC-6 (M/H), AC-6(9) (M/H), AC-6(10) (M/H), IA-5 (L/M/H), CM-5 (M/H) | AC-2 (Account Management), AC-3 (Access Enforcement), AC-6 (Least Privilege), AC-6(9), AC-6(10), IA-5 (Authenticator Management), CM-5 (Access Restrictions for Change) |
| Layer 3: Encryption | SC-28(1): customer-managed CMK is the agency-managed key CJIS requires for CJI at rest; an AWS-managed key would place AWS personnel in CJIS scope | SC-12 (L/M/H), SC-13 (L/M/H), SC-28 (M/H), SC-28(1) (M/H) | SC-12 (Cryptographic Key Establishment and Management), SC-13 (Cryptographic Protection), SC-28 (Protection of Information at Rest), SC-28(1) (Cryptographic Protection) |
| Layer 4: Configuration & Compliance | Strict S3KmsEncryptionRule checks SSE-KMS (not SSE-S3) because the AWS-managed key fails the SC-28(1) agency-managed-key requirement | CM-2 (L/M/H), CM-6 (L/M/H), CM-8 (L/M/H), SC-7 (L/M/H) | CM-2 (Baseline Configuration), CM-6 (Configuration Settings), CM-8 (System Component Inventory), SC-7 (Boundary Protection; RestrictedSshRule provides continuous detection of 0.0.0.0/0 SSH exposure, complementing the SCP's preventive control) |
| Layer 5: Detection & Response | IR-5/IR-6 only: SNS/SQS alert path encrypts with the agency-managed CMK when the Layer 3 ARN is supplied, otherwise AWS-managed keys | SI-3 (L/M/H), SI-4 (L/M/H), IR-4 (L/M/H), IR-5 (L/M/H), IR-6 (L/M/H) | SI-3 (Malicious Code Protection), SI-4 (System Monitoring), IR-4 partial (detection and notification only; containment, eradication, and recovery are out of scope), IR-5 (Incident Monitoring), IR-6 (Incident Reporting) |
| Secure S3 Bucket | Does not meet SC-28(1): AES256 (SSE-S3) is not an agency-managed key; not suitable for CJI at rest as-is | SC-28 (M/H), AC-3 (L/M/H) | SC-28 (Protection of Information at Rest), AC-3 (Access Enforcement) |

> **Note:** CJIS v6.x uses NIST 800-53 control identifiers directly, so repeating the IDs in a CJIS column would say nothing. The CJIS column instead carries only the deltas: where CJIS demands more than the FedRAMP High baseline (agency-managed keys, AAL2 MFA, FIPS-validated crypto for CJI) and where this baseline falls short of that. `n/a` means the requirement coincides with the 800-53 control already listed.
